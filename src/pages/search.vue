              ...
              <el-dropdown slot="prepend" @command="dropdownClick">
                ...
                <el-dropdown-menu slot="dropdown">
                  <el-dropdown-item
                    ...
                    :key="index"
                    :command="searchType"
                  >{{searchType.value}}</el-dropdown-item>
                </el-dropdown-menu>
              </el-dropdown>
          ...
          <section class="selection_cnt">
            <el-row type="flex" class="row_bottom_line">
              ...
              <el-col :span="21">
                ...
                >
                  <el-checkbox
                    v-for="(showCategory, index) in showCategories"
                    :key="index"
                    :label="showCategory.id"
                    border
                    size="medium"
                    style="margin-right: 10px; margin-bottom: 10px;"
                  >{{showCategory.title}}</el-checkbox>
                </el-checkbox-group>
                <el-autocomplete
                  :fetch-suggestions="queryCategories"
                  @select="addCategories"
                  v-model="category"
                  placeholder="搜索类目"
                  size="medium"
                  class="input"
            ...
            <el-row type="flex" class="row_bottom_line">
              ...
              <el-col :span="21">
                ...
                >
                  <el-checkbox
                    v-for="(showTag, index) in showTags"
                    :key="index"
                    :label="showTag.id"
                    border
                    size="medium"
                    style="margin-right: 10px; margin-bottom: 10px;"
                  >{{showTag.title}}</el-checkbox>
                </el-checkbox-group>
                <el-autocomplete
              ...
              <el-col :span="21">
                ...
                >
                  <el-checkbox
                    ...
                    :key="index"
                    style="margin-bottom: 10px;"
                  >{{type.key + '(' + type.doc_count +')'}}</el-checkbox>
                </el-checkbox-group>
              </el-col>
            </el-row>
            ...
            <el-row type="flex" class="row_bottom_line">
              ...
              <el-col :span="21">
                <el-radio-group
                  @change="!loading && defaultCTime != CTimes.length - 1 && search()"
                  v-model="defaultCTime"
                >
                  <el-radio
                    v-for="(ct, index) in CTimes"
                    :key="index"
                    :label="index"
                    style="margin-bottom: 10px;"
                  >{{ct.key + '(' + ct.doc_count +')'}}</el-radio>
                </el-radio-group>

                <el-date-picker
                  v-show="defaultCTime == CTimes.length - 1"
                  v-model="CTime"
                  type="daterange"
                  range-separator="-"
            ...
            <el-row type="flex" class="row_top_line">
              ...
              <el-col :span="21">
                <el-radio-group
                  @change="!loading && defaultCTime != CTimes.length - 1 && search()"
                  v-model="defaultETime"
                >
                  <el-radio
                    v-for="(et, index) in ETimes"
                    :key="index"
                    :label="index"
                    style="margin-bottom: 10px;"
                  >{{et.key + '(' + et.doc_count +')'}}</el-radio>
                </el-radio-group>

                <el-date-picker
                  v-show="defaultETime == ETimes.length - 1"
                  v-model="ETime"
                  type="daterange"
                  range-separator="-"
                  start-placeholder="开始日期"
          ...
          <section class="contain_cnt">
              ...
              >
                ...
                <div class="popover_content">
                  ...
                  <el-row style="margin-top: 10px;">
                    ...
                    <el-col :offset="2" :span="16">
                      <span
                        v-for="(desc, index) in item.desc_highlights"
                        :key="index"
                        v-html="desc"
                      >{{desc}}</span>
                    </el-col>
                  ...
                  <el-row style="margin-top: 10px;" v-if="item.content_highlights != null">
                    ...
                    <el-col :offset="2" :span="16">
                      <span
                        v-for="(content, index) in item.content_highlights"
                        :key="index"
                        v-html="content"
                      >{{content}}</span>
                    </el-col>
            ...
            <div v-if="isList" class="card_cnt">
              <el-row
                v-for="(item, index) in resultItems"
                :key="index"
                style="height: 220px; width: 100%; padding: 10px; overflow: hidden;"
              >
                <el-col :span="5">
        ...
        created_time = {
          from:
            this.CTimes[this.defaultCTime] != null
              ? this.CTimes[this.defaultCTime].from_as_string
              : null,
          to:
            this.CTimes[this.defaultCTime] != null
        ...
        modified_time = {
          from:
            this.ETimes[this.defaultETime] != null
              ? this.ETimes[this.defaultETime].from_as_string
              : null,
          to:
            this.ETimes[this.defaultETime] != null
          ...
          if (res.data.status === 200) {
            ...
            for (let j = 0; j < res.data.data.group_by_created_time.length; ++j)
              if (
                ...
                _this.CTimes[_this.defaultCTime] != null &&
            ...
            for (
              ...
              if (
                !_this.isKeywordChanged &&
                _this.ETimes[_this.defaultETime] != null &&
