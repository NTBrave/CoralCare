...
const mutations = {
  ...
  setCoralNumberStatistic(state, coralNumberStatistic) {
    ...
    for (let i = 0; i <= coralNumberStatistic.length - 1; i++) {
      ...
      if (coralNumberStatistic[i].now_alive_number != undefined) {
        obj[coralNumberStatistic[i].key][0] =
          coralNumberStatistic[i].now_alive_number;
      }
      if (coralNumberStatistic[i].history_alive_number != undefined) {
        obj[coralNumberStatistic[i].key][1] =
          coralNumberStatistic[i].history_alive_number;
      }
      if (coralNumberStatistic[i].history_death_number != undefined) {
        obj[coralNumberStatistic[i].key][2] =
          coralNumberStatistic[i].history_death_number;
      }
    }
  ...
  // 缓存作业下的活动列表（用于新建作业）
  setActivityList(state, activity) {
